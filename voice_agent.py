...
class VoiceAgent:
    def __init__(self, debug: bool = True):
        ...
        self.INACTIVITY_TIMEOUT = 20.0
        self.commit_event = asyncio.Event()  # ← signal for commit
    ...
    def on_button_release(self):
        ...
        self.commit_event.set()  # ← request commit in async loop
        self.last_activity = time.monotonic()

        if self.debug:
            print("RELEASE → commit requested")
                        ...
                        while True:
                            ...
                            if self.commit_event.is_set():
                                try:
                                    ...
                                    if self.debug: print("Commit sent")
                                except Exception as e:
